...
export function dropEnum(args) {
    return args.queryInterface.sequelize.query(getQueryToDropEnum(args.name), args.sequelizeOptions);
}
...
export function setColumnDefault(args) {
    return args.queryInterface.sequelize.query(
        getQueryToSetColumnDefault(args.tableName, args.columnName, args.defaultValue, args.defaultValueType),
        args.sequelizeOptions
    );
}
